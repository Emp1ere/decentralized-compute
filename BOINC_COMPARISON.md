...
**Цикл воркера (одна задача vs много):** В BOINC клиент работает как демон: после старта он многократно запрашивает работу, выполняет и отчитывается; остановка — через «Suspend» или «No new tasks». В нашем проекте воркер реализован так же: один запуск = цикл «get_task → вычисление → submit_work → пауза → снова get_task», пока процесс не остановить. Это соответствует концепции BOINC. При необходимости «один клик = одна задача» можно включить через переменную окружения (например, `RUN_ONCE=1`).
