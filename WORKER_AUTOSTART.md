...
**Кратко для обывателя:** запустите Docker Desktop → дважды нажмите **start.bat** → в открывшемся интерфейсе зарегистрируйтесь, выберите задачу, нажмите **«Запустить воркер автоматически»**. Подробно: см. **START_ПРОСТО.md**.
...
Образ воркера (`distributed-compute_client_worker_1`) должен существовать. В папке проекта выполните:
...
Или один раз запустите `docker-compose up --build` — соберутся все сервисы, включая воркер. Без сборки кнопка «Запустить воркер автоматически» выдаст ошибку «Image not found».
...
1. Запустите **Docker Desktop** и дождитесь, пока он полностью поднимется (иконка в трее без «Starting…»).
2. Дополнительно настраивать сокет не нужно: при монтировании `/var/run/docker.sock` в `docker-compose.yml` Docker Desktop сам обеспечивает доступ контейнера к демону.
3. Откройте терминал в каталоге проекта и выполните:
   ```bash
   docker-compose up --build
   ```
4. Когда контейнеры запустятся, откройте в браузере http://localhost:5000 и используйте кнопку «Запустить воркер автоматически» — сокет уже доступен оркестратору.
